...
---
Updated by David E. Weirich for Bill Weirich -- Aug. 2020
"""
...
class ImageGui:
    ...
    def __init__(self, master, labels, paths):
        ...
        for ll, button in enumerate(self.buttons):
            r = ll // 5
            c = ll % 5
            button.grid(row=r, column=c, sticky="we")
            frame.grid_columnconfigure(ll, weight=1)

        # Place progress label in grid
        self.progress_label.grid(
            row=0, column=self.n_labels + 2, sticky="we"
        )  # +2, since progress_label is placed after
        ...
        self.sorting_label = tk.Label(
            frame, text=("in folder: %s" % (sorting_string)), width=15
        )
        ...
        self.sorting_label.grid(
            row=2, column=self.n_labels + 1, sticky="we"
        )  # +2, since progress_label is placed after
        # and the additional 2 buttons "next im", "prev im"
        # Place the image in grid
        self.image_panel.grid(
            row=r+1, column=0, columnspan=self.n_labels + 1, sticky="we"
        )
    ...
    def show_next_image(self):
        ...
        #### added in version 2
        sorting_string = os.path.split(df.sorted_in_folder[self.index])[
            -2
    ...
    def move_prev_image(self):
        ...
        if self.index < self.n_paths:
            self.set_image(
                df.sorted_in_folder[self.index]
            )  # change path to be out of df
        else:
            self.master.quit()
